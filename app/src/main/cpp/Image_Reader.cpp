 ...
 *   Increased to 4 to provide more breathing room for the camera pipeline.
 */
...
void Image_Reader::ImageCallback(AImageReader *reader) {
    // We don't need to do much here for YUV if we are using acquireLatestImage
    // in the processing loop. However, for JPEG we might want to handle it.
    int32_t format;
